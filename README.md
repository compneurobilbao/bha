...
repository into your computer and follow the instructions at the [bottom of this page](#using-bha-as-a-template).
...
`bha/bha.py`. This code provided in this file is intentionally rather
...
We place the module code in a file called `bha.py` in directory called
`bha`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import bha as sb` in an
interactive Python session, the classes and functions defined inside of the
`bha.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the bha module, the main classes defined at the bottom of
...
Either way, you can create a `bha/data` folder in which you can
...
(typically, something like our `bha/tests/test_bha.py`). Within each
...
have one such test in `bha/tests/test_bha.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/bha`),
and type:
...
will see a message such as:
  ...
  bha/tests/test_bha.py:49: AssertionError
...
To document `bha` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/bha/blob/master/doc/_static/RTD-advanced-conf.png)

 http://bha.readthedocs.org/en/latest/
...
A `bha/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/bha) for the software. This
...
`bha` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `bha`, we use the
...
[here](https://github.com/uwescience/bha/blob/master/scripts/Figure1.ipynb).


### Using `bha` as a template
...
computer under the name you will want your project to have:
	...
	git clone https://github.com/uwescience/bha smallish
	cd smallish
...
Next, you will want to move `bha/bha.py` to be called
`smallish/smallish.py`

	git mv bha smallish
	git mv smallish/bha.py smallish/smallish.py
	git mv smallish/tests/test_bha.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `bha` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `bha` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `bha` in them. Type
the following to see where all these files are:

	git grep bha

You can replace `bha` for `smallish` fastly with:

	git grep -l 'bha' | xargs sed -i 's/bha/smallish/g'
...
Edit all the mentions of `bha` in the `bha/__init__.py` file, and in
the `bha/version.py` file as well.
